 ...
 *
 * <p>Error Distance Approach for Drift Detection and Monitoring (EDIST2). 
 * EDIST2 was proposed in order to deal with these complex drifts
 * EDIST2 monitors the learner performance through a self-adaptive window
 * that is autonomously adjusted through a statistical hypothesis test. This
 * statistical test provides theoretical guarantees, regarding the false alarm
 * rate, which were experimentally confirmed. </p>
 *
 * <p>See details in:<br> Imen Khamassi, Moamar Sayed-Mouchaweh, Moez Hammami
 * Self-Adaptive Windowing Approach for Handling Complex Concept Drift. 
 * In Cognitive Computation, DOI: 10.1007/s12559-015-9341-0, Springer, 2015.</p>
 *
 * <p>Parameters:</p> <ul>
...
public class EDIST2 extends AbstractClassifier implements MultiClassClassifier {
    ...
    private void updateParatemeters(WindowEDIST2 tglobal, WindowEDIST2 tcurrent)
    {
        ...
        tglobal.delta = Math.sqrt((((tglobal.N * Math.pow(tglobal.delta, 2)) + 
                                  (tcurrent.N * Math.pow(tcurrent.delta, 2))) / 
    ...
    /**
     * Inner class that represents a single data window characterized by:
     * error number (N), error distance mean (mu) and error distance standard 
     * deviation (delta).
     */
    protected final class WindowEDIST2 extends AbstractMOAObject 
    {
        ...
        public void updateParameters(long indexError)
        {
            ...
            this.delta = Math.sqrt(
                                    (N - 1) * Math.pow(this.delta, 2) / (this.N + Double.MIN_VALUE) +
